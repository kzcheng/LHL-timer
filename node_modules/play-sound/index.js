...
function Play(opts) {
  ...
  this.play = function(what, options, next) {
    ...
    let isURL = this.player == 'mplayer' && this.urlRegex.test(what);
    ...
    let args = Array.isArray(options[this.player]) ? options[this.player].concat(what) : [what];
    let process = spawn(this.player, args, options);
    if (!process) {
